...
\subsection{Aplicativo Híbrido} % (fold)
\label{sub:aplicativo_hibrido}
...
%%%%% dividir ???
Inicialmente tentou-se utilizar sistemas prontos ou de fácil uso para a criação da ferramenta de coleta de dados. Há ferramentas disponíveis gratuitamente para auxiliar a construção de aplicativos para dispositivos móveis. Estes, em grande parte, seguem o princípio aberto-fechados \cite{meyer1988:open-closed}, onde uma pessoa com conhecimento podem estender as funcionalidades principais, mas não modificá-las diretamente. 

Um aplicativo para a ferramenta EpiCollect foi criado e um outro para a ferramenta Sensr.



\subsection{Sistemas de Coletas de Dados} % (fold)
\label{sub:sistemas_de_coletas_de_dados}

% subsection sistemas_de_coletas_de_dados (end)




\subsection{Infraestrutura Tecnológica} % (fold)
