...
Este sistema é de fácil uso, bastando ter acesso a internet para definir um nome para o projeto, especificar a visibilidade do projeto se público ou privado, se este poderá aparecer na página da internet (Figura \ref{fig:epicollect_novo_projeto}) e criar um formulário (Figura \ref{fig:epicollect_formulario}). Foi desenvolvido um formulário baseado nos principais requisitos do projeto no qual visa captar áudio, vídeo e imagem juntamente com a coordenada geográfica atual do voluntário.
...
Para validação deste aplicativo um formulário foi criado utilizando a interface gráfica oferecido pela página do EpiCollect+ (Figura \ref{fig:epicollect_formulario}). Após efetuar o downlaod do aplicativo 

Para a validação deste sistema, foi criado um formulário onde o voluntário registra a data e hora em que está utilizando o aplicativo, selecionar se esta coleta será uma captura de image, vídeo ou áudio, então é apresentado ao voluntário a tela de captura conforme a seleção feita (Figura \ref{fig:epicollect_formulario}). Desta forma, fica fácil filtrar os dados pelo tipo de escolha do voluntário.

\begin{figure*}[htb]
\centering
\includegraphics[keepaspectratio,width=0.9\textwidth]{figuras/epicollect/04_FORM_CREATION.png}
