...
\subsection{Aplicativo de Sistemas Prontos} % (fold)
\label{ssub:aplicativo_com_sistemas_prontos}

Alguns aplicativos tornam-se poderosas ferramentas de coleta de dados, inclusive adicionando as informações de alguns dos sensores presentes no dispositivo. Estes utilizam formulários para guiar a coleta de dados, formulários que são construídos através de interfaces gráficas de fácil manipulação. Estes sistemas prontos são indicados para efetuar a criação aplicativos voltado para pesquisas ou coleta de dados de uma forma rápida, uma vez que estas possuem toda uma infraestrutura para armazenamento de dados e sua visualização.

EpiCollect+ é a segunda geração de um conhecido sistema pronto e possui diversos novos recurso do que a sua primeira geração \cite{Aanensen2009:epicollect}, como capturar imagens, vídeos, áudios, localização geográfica e leitura de código de barra.

Este sistema é de fácil uso, bastando ter acesso a internet para definir um nome para o projeto, especificar a visibilidade do projeto se público ou privado, se este poderá aparecer na página da internet (Figura \ref{fig:epicollect_novo_projeto}) e criar um formulário (Figura \ref{fig:epicollect_formulario}).  

%Foi desenvolvido um formulário baseado nos principais requisitos do projeto no qual visa captar áudio, vídeo e imagem juntamente com a coordenada geográfica atual do voluntário.
...
\caption{À esquerda, página apresentada para a criação de um novo projeto usando o EpiCollect+, opcionalmente pode ser informado um proejto do primeiro EpiCollect para transformá-lo em um novo. À direita,  escolha da visibilidade do projeto, se este poderá ser visível a todos ou se os usuários deverão cadastrar-se no projeto. Por último, a opção se o projeto poderá ser listado no site do EpiCollect+. }
...
\begin{figure}[htb]
\centering
\includegraphics[keepaspectratio,width=0.7\textwidth]{figuras/pertinencia.png}
...
Diferente da primeira versão do sistema, o EpiCollect+ possui apenas aplicativo para a plataforma Android. Não justificando a hipótese supracitada no início da seção \ref{sub:aplicativos} onde aplicativos criados somente para uma plataforma são fatores inibidores e atraem menos voluntários. 

Outra plataforma de sistemas prontos foi avaliado, Sensr, porém só existe aplicativo para a plataforma iOS. Assim estes dois protótipos foram criados, mas não utilizados devido a falta de estrutura apresentados pelos sistemas pronto em realação a mais de uma plataforma.

\subsection{Aplicativo de Biblioteca Nativa} % (fold)
\label{ssub:aplicativo_com_biblioteca_nativa}

Após verificar a utilização de um aplicativo de sistemas prontos, optou-se por desenvolver um aplicativo para cada plataforma, utilizando as suas próprias ferramentas. Cada fabricante de dispositivos móveis disponibiliza conjuntos de códigos a serem utilizados para desenvolver aplicativos, estes são conhecido por biblioteca, \textit{API} ou \textit{SDK}. A maioria dos fabricantes disponibilizam essas bibliotecas sem custos, porém, para poder publicar um aplicativo nas lojas online, alguns fabricantes requerem uma assinatura anual, Tabela \ref{tab:tabela_custo}.
...
Na Tabela \ref{tab:tabela_custo}, as plataformas mostradas são referentes a pesquisa realizada por \citeonline{IDC:2014Q3}, onde somente as plataformas mais populares de 2011 ao penúltimo trimestre de 2014 são discriminadas em intervalos trismestrais.
...
A plataforma Windows Phone foi escolhida para criar o aplicativo por já haver um conhecimento prévio com a linguagem utilizada pelo framework da Microsoft. O aplicativo foi testado em um aparelho Lumia Nokia 800, lançado no final de 2011\footnote{\url{http://www.gsmarena.com/nokia_lumia_800-4240.php}}, e com sensores de GPS, câmera de 8 megapixel, acelerômetro e bússola magnética.

Este aplicativo tinha a simples função de utilizar as funções já dispoíveis da plataforma, captar uma imagen e verificar se a mesma possuia um cabeçalho Exif \cite{JEITA2002} válido georeferenciado. 




% Seria então necessário novos paradigmas para receber e armazernar os dados enviados deste novo aplicativo, Figura \ref{fig:wp7}.
% Um servidor Asp.Net MVC foi desenvolvido para receber os dados utilizando-se RESTful e exibir os resultados captados dos voluntários, mostrando à esquerda um mapa em HTML5 provido pelo Bing e á direita a imagem captada com as propriedades captadas logo abaixo, como número de identificação da imagem, posição geográfica (latitude e longitude), precisão da posição geográfica e data em que a imagem foi captada. Um banco de dados postgresql 9.1 foi utilizado para armazenar os dados em formato GeoJSON.
% \begin{figure}[htb]
% \centering
