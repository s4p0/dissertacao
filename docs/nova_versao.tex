...
No meio da década de 90, surgiu os projetos \textbf{\textit{GIMPS}}\footnote{\textit{\url{http://www.mersenne.org/}-Great Internet Mersenne Prime Search}} e \textbf{\textit{Distributed.net}}\footnote{http://www.distributed.net/}\cite{Anderson1999,AndersonCKLW02,Hayes1998}. Com a ferramenta BOINC \cite{Anderson1999} a computação voluntária ganhou força e popularidade, chegando a ser utilizada por mais de 100 projetos\cite{anderson2003public}. 

Na década seguinte, outra forma de realizar ciência cidadã moderna foi destacada. Reunindo diversos cidadãos para olhar para o espaço, o projeto GalaxyZoo\cite{oxfordgalaxy2007} ganhou grande notoriedade por conseguir 1.5 milhões de classificações de galáxias realizadas por voluntários. Assim como na computação voluntária, o projeto recebeu apoio de cidadãos ordinários. 

Essas duas formas de ciência cidadã são diferentes entre si, onde a computação voluntária necessita apenas de uma conexão para interligar os computadores e potência computacional para auxiliar projetos. Porém a última, conhecida por pensamento voluntário, necessita apenas do poder cognitivo do cidadão, para que esse possa auxiliar os projetos deste tipo com tomadas de ação e decisões que não seriam fácil e nem rápido para uma máquina resolver, assim como auxiliar na forma de reconhecimento de padrões. A cada dia há um novo \textit{hardware} sendo aprimorado, mas nenhum supera o ser humano em questão de reconhecimento visual.

Outra forma de realizar projetos de ciência cidadã vem moldando-se ao longo dos últimos anos, diferente de utilizar somente a capacidade cognitiva ou identificação de padrões e diferente de utilizar o tempo de processamento ocioso das máquinas, tem-se os projetos que necessitam da contribuição dos voluntários para fornecer dados. Um dos projetos mais antigos de ciência cidadã e ainda em vigor, o Christmas Bird Count, cativa milhares de pessoas até hoje, na sua 115\textdegree edição, todos os anos.

O projeto ForestWatchers é o primeiro projeto que se tem conhecimento que almeja utilizar-se destas três formas de fazer ciência cidadã para abordar desafios até então inconcebíveis, como realizar o monitoramento das Florestas Tropicais em volta do globo. Tal desafio precisa de novos paradigmas, sistemas antiquados não suportam tamanha escalabilidade de monitorar florestas em diferentes partes do mundo. O objetivo deste projeto é avaliar de uma forma rigorosamente científica os benefícios e limitações da participação do público através da Web neste tipo de exercício, comparando os resultados obtidos a partir de muitos voluntários.
...
\begin{enumerate}
    ...
    \item Propor e desenvolver um módulo de sensoriamento voluntário.
