...
A infraestrutura para receber, armazenar e exibir os dados coletados pelos voluntários utilizou-se de tecnologias de código aberto para a sua construção com a finalidade de obter-se uma plataforma de distribuição livre juntamente com seu código fonte, podendo ser enriquecido por outros trabalhos.
%colocar a figura mais próximo%
%muita figura e tabela para tal%
...
Este é o princípio do serviço web conhecido por \textit{Representational State Transfer} (REST), em português Transferência de Estado Representacional, onde o serviço é estilo arquitetural baseado no protocolo HTTP projetado para utilizar um conjunto de operações bem definidas utilizando os principais métodos HTTP (Tabela \ref{tab:metodos_http}), um protocolo cliente-servidor sem estado, um Identificador Uniforme de Recursos (Uniform Resource Identifier - URI) para um determinado objeto e transferência de dados, podendo utilizar o formato JSON.
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{Áudio.} Representação de um dado de áudio que ao clicar invoca uma janela modal mostrando todas as suas propriedades.
    ...
    \item \textbf{Foto.} Representação de um dado de foto que ao clicar invoca uma janela modal mostrando todas as suas propriedades.
...
Porém, utilizar imagens SPOT, como é o caso deste trabalho, para comparar dois tipos de imagens em diferentes camadas não é viável. Imagens SPOT são pagas, diferente das imagens MODIS. Portanto a alternativa levantada pela hipótese deste trabalho sugere que imagens, vídeos ou áudios captados in-situ possam ajudar a discenir uma área florestada de uma área não-florestada. 
...
Para validar o módulo proposto neste trabalho o aplicativo foi utilizado em uma área do Estado do Pará, próximo a Belterra. O aplicativo captou fotos e vídeo para serem sincronizados posteriormente com o módulo web, também desenvolvido neste trabalho. Nesta etapa, não foi possível validar o sistema de captura de áudio devido a um bug encontrado na versão do PhoneGap para o sistema Windows Phone e por a área não possuir comunicação com Internet, não sendo possível o contorno do problema a tempo. Os dados apresentados neste trabalho para o áudio, são representativos e foram gerados a partir da extração do áudio dos arquivos de vídeos captados.
