...
Atualmente, são utilizadas imagens disponíveis no catálogo\footnote{\url{https://earthdata.nasa.gov/earth-observation-data/near-real-time/rapid-response/modis-subsets}} gratuíto de imagens da Terra oferecido pela NASA. Estas imagens são disponíveis nos formatos \textit{GeoTIFFs}, \textit{JPEGs} ou \textit{KMZ}, para RGB Cores Verdadeiras (\textit{True-Color RGB}). O sítio eletrônico também disponibiliza as imagens em outras bandas, possibilitando perceber diferentes características. Porém, para que essas imagens possam ser exibidas para os voluntários é necessário que um pré-processamento seja realizado.
...
% \includegraphics[scale=0.7]{figuras/resultados/map.png}
